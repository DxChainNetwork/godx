...
func ProcessVote(state stateDB, ctx *types.DposContext, addr common.Address, deposit common.BigInt,
	candidates []common.Address, time int64, blockNumber int64, config *params.DposConfig) (int, error) {
	...
	if deposit.Cmp(prevDeposit) < 0 {
		...
		markThawingAddressAndValue(state, addr, epoch, diff, blockNumber, config)
...
func ProcessCancelVote(state stateDB, ctx *types.DposContext, addr common.Address, time int64, blockNumber int64, config *params.DposConfig) error {
	if err := ctx.CancelVote(addr); err != nil {
		return err
	}
	prevDeposit := GetVoteDeposit(state, addr)
	currentEpoch := CalculateEpochID(time)
	markThawingAddressAndValue(state, addr, currentEpoch, prevDeposit, blockNumber, config)
