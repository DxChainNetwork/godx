...
// Node Record: set up public key, create signature and stored into node record
...
func (V4ID) Verify(r *enr.Record, sig []byte) error {
	var entry s256raw
	// check if IP stored in the record provided is valid
	...
	h := sha3.NewLegacyKeccak256()
	// encode the entries and sequence getting from node record
...
// Node address is derived from its' public key
// using Keccak256 encoding rule
...
// set public key in Record.pairs
// set empty signature
...
// set everything to null, including
// node ID, node record entries, node record signature
