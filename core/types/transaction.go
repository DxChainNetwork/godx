...
// Notice this function will return a copy of the original Transaction.
...
//
// The input txs is a map of transactions with key of from address.
// The head is the first element of transactions with different keys.
// This structure is used pulling out the transaction with the largest Price in the head list.
func NewTransactionsByPriceAndNonce(signer Signer, txs map[common.Address]Transactions) *TransactionsByPriceAndNonce {
	// Initialize a price based heap with the head transactions
	heads := make(TxByPrice, 0, len(txs))
	for from, accTxs := range txs {
		// heads: head transactions. One transaction for each address.
