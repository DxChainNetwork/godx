...
import (
	...
	"github.com/DxChainNetwork/godx/storage/maintenance"
)
...
func TestEVM_CreateContractTx(t *testing.T) {
	...
	statusAddr := common.BytesToAddress([]byte(maintenance.StrPrefixExpSC + windowEndStr))
	...
	if !bytes.Equal(statusFlag.Bytes()[11:12], maintenance.NotProofedStatus) {
		t.Errorf("write wrong contract status into state,wanted %v,getted %v", maintenance.NotProofedStatus, statusFlag.Bytes())
	}

	clientCollateral := sc.ClientCollateral.Value
	hostCollateral := sc.HostCollateral.Value

	clientCollateralHash := stateDB.GetState(contractAddr, maintenance.KeyClientCollateral)
	...
	hostCollateralHash := stateDB.GetState(contractAddr, maintenance.KeyHostCollateral)
	...
	fileSizeHash := stateDB.GetState(contractAddr, maintenance.KeyFileSize)
	...
	unlockHash := stateDB.GetState(contractAddr, maintenance.KeyUnlockHash)
	if unlockHash != sc.UnlockHash {
		t.Errorf("write wrong unlock hash data into state,wanted %v,getted %v", sc.UnlockHash, unlockHash)
	}

	fileMerkleRoot := stateDB.GetState(contractAddr, maintenance.KeyFileMerkleRoot)
	...
	revisionNumHash := stateDB.GetState(contractAddr, maintenance.KeyRevisionNumber)
	...
	windowStartHash := stateDB.GetState(contractAddr, maintenance.KeyWindowStart)
	...
	windowEndHash := stateDB.GetState(contractAddr, maintenance.KeyWindowEnd)
	...
	clientVpoHash := stateDB.GetState(contractAddr, maintenance.KeyClientValidProofOutput)
	...
	hostVpoHash := stateDB.GetState(contractAddr, maintenance.KeyHostValidProofOutput)
	...
	clientMpoHash := stateDB.GetState(contractAddr, maintenance.KeyClientMissedProofOutput)
	...
	hostMpoHash := stateDB.GetState(contractAddr, maintenance.KeyHostMissedProofOutput)
	...
	clientAddressHash := stateDB.GetState(contractAddr, maintenance.KeyClientAddress)
	...
	hostAddressHash := stateDB.GetState(contractAddr, maintenance.KeyHostAddress)
...
func TestEVM_StorageProofTx(t *testing.T) {
	...
	if !bytes.Equal(statusHash.Bytes()[11:12], maintenance.ProofedStatus) {
...
func mockWriteStorageContractIntoState(sc types.StorageContract, state *state.StateDB) {
	...
	state.SetState(statusAddr, scID, common.BytesToHash(append(maintenance.NotProofedStatus, contractAddr[:]...)))

	// store storage contract in this contractAddr's state
	state.SetState(contractAddr, maintenance.KeyClientAddress, common.BytesToHash(sc.ClientCollateral.Address.Bytes()))
	state.SetState(contractAddr, maintenance.KeyHostAddress, common.BytesToHash(sc.HostCollateral.Address.Bytes()))

	state.SetState(contractAddr, maintenance.KeyClientCollateral, common.BytesToHash(sc.ClientCollateral.Value.Bytes()))
	state.SetState(contractAddr, maintenance.KeyHostCollateral, common.BytesToHash(sc.HostCollateral.Value.Bytes()))

	uintBytes := Uint64ToBytes(sc.FileSize)
	state.SetState(contractAddr, maintenance.KeyFileSize, common.BytesToHash(uintBytes))

	state.SetState(contractAddr, maintenance.KeyUnlockHash, sc.UnlockHash)
	state.SetState(contractAddr, maintenance.KeyFileMerkleRoot, sc.FileMerkleRoot)

	uintBytes = Uint64ToBytes(sc.RevisionNumber)
	state.SetState(contractAddr, maintenance.KeyRevisionNumber, common.BytesToHash(uintBytes))

	uintBytes = Uint64ToBytes(sc.WindowStart)
	state.SetState(contractAddr, maintenance.KeyWindowStart, common.BytesToHash(uintBytes))

	uintBytes = Uint64ToBytes(sc.WindowEnd)
	state.SetState(contractAddr, maintenance.KeyWindowEnd, common.BytesToHash(uintBytes))

	state.SetState(contractAddr, maintenance.KeyClientValidProofOutput, common.BytesToHash(sc.ValidProofOutputs[0].Value.Bytes()))
	state.SetState(contractAddr, maintenance.KeyHostValidProofOutput, common.BytesToHash(sc.ValidProofOutputs[1].Value.Bytes()))

	state.SetState(contractAddr, maintenance.KeyClientMissedProofOutput, common.BytesToHash(sc.MissedProofOutputs[0].Value.Bytes()))
	state.SetState(contractAddr, maintenance.KeyHostMissedProofOutput, common.BytesToHash(sc.MissedProofOutputs[1].Value.Bytes()))
}
