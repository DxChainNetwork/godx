...
// This function is similar to types.bloom9 function. The difference is that this
// function only calculate the position of three 1s instead of giving a byte slice
...
// For current code in eth/filters/filter.go:90, the filters have only one filter,
...
func NewMatcher(sectionSize uint64, filters [][][]byte) *Matcher {
	...
	// Change the filter of bytes to m.filters of index
	for _, filter := range filters {
		...
		for i, clause := range filter {
			// If one of the address in filter is nil, no filter rules
			if clause == nil {
				bloomBits = nil
				break
			}
...
// channel is closed.
// results is channel of size 64 in current code. ctx define
func (m *Matcher) Start(ctx context.Context, begin, end uint64, results chan uint64) (*MatcherSession, error) {
	...
	sink := m.run(begin, end, cap(results), session) // cap(results) == 64
...
func (m *Matcher) subMatch(source chan *partialMatches, dist chan *request, bloom []bloomIndexes, session *MatcherSession) chan *partialMatches {
	...
	for i, bits := range bloom {
		for j, bit := range bits {
			sectionSources[i][j] = make(chan uint64, cap(source)) // requests
			sectionSinks[i][j] = make(chan []byte, cap(source))   // results
			// dist: processing requests
...
func (s *MatcherSession) Multiplex(batch int, wait time.Duration, mux chan chan *Retrieval) {
	...
	for {
		...
		case mux <- request:
			...
			// request will not request a read immediately?
			result := <-request
			if result.Error != nil {
				s.err.Store(result.Error)
				s.Close()
			}
			// result.Bitsets is the data read from Canonical chain
