...
// ExampleStateDB_Normal execute the normal GET - SET - COMMIT operation
func ExampleStateDB_Normal() {
	db := NewDatabase(ethdb.NewMemDatabase())
	s, _ := New(common.Hash{}, db)

	// Create and add an address
	acc := common.HexToAddress("0x1234567890")
	s.CreateAccount(acc)
	s.AddBalance(acc, big.NewInt(1))
	s.Finalise(true)

	// Add some balance, and commit the state, the data shall be written to database
	s.AddBalance(acc, big.NewInt(2))
	root, _ := s.Commit(true)

	// Create a new trie to see whether the data has be written to database
	s, _ = New(root, db)
	balance := s.GetBalance(acc)
	fmt.Printf("The account %s have %v balance", acc.Hex(), balance)
	// Output:
	// The account 0x0000000000000000000000000000001234567890 have 3 balance
}

// ExampleStateDB_Revert gives and example of how to revert the StateDB
func ExampleStateDB_Revert() {
	db := NewDatabase(ethdb.NewMemDatabase())
	s, _ := New(common.Hash{}, db)

	// snapshot
	shotId := s.Snapshot()
	// Create an address and add balance
	acc := common.HexToAddress("0x1234567890")
	s.CreateAccount(acc)
	s.AddBalance(acc, big.NewInt(1))

	// Revert to the initial state
	s.RevertToSnapshot(shotId)

	balance := s.GetBalance(acc)
	fmt.Printf("The account %s have %v balance", acc.Hex(), balance)
	// Output:
	// The account 0x0000000000000000000000000000001234567890 have 0 balance
}
