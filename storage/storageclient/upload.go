...
func (sc *StorageClient) Upload(up storage.FileUploadParams) error {
	...
	dxDirEntry, err := sc.fileSystem.NewDxDir(dirDxPath)
	...
	entry, err := sc.fileSystem.NewDxFile(up.DxPath, storage.SysPath(up.Source), false, up.ErasureCode, cipherKey, uint64(sourceInfo.Size()), sourceInfo.Mode())
	if err != nil {
