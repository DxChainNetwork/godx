...
func sendStorageContractCreateTx(storageContract types.StorageContract, clientPaymentAddress common.Address, cp clientnegotiation.ContractRenewProtocol) error {
...
func clientStorageContractCommit(cp clientnegotiation.ContractRenewProtocol, sp storage.Peer, enodeID enode.ID, startHeight uint64, funding common.BigInt, contractPrice common.BigInt, contractID common.Hash, contractRevision types.StorageContractRevision, oldContract *contractset.Contract) (storage.ContractMetaData, error) {
...
func handleContractRenewErr(cp clientnegotiation.ContractRenewProtocol, err *error, hostID enode.ID, sp storage.Peer) {
	handleNegotiationErr(cp, err, hostID, sp, storagehostmanager.InteractionRenewContract)
}
