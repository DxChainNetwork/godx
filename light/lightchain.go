...
func (lc *LightChain) DposCtx() (*types.DposContext, error) {
	return nil, errors.New("not implemented")
}

// DposCtxAt returns the dposCtx given the dpos roots
func (lc *LightChain) DposCtxAt(roots *types.DposContextRoot) (*types.DposContext, error) {
	return nil, errors.New("not implemented")
}
...
func (lc *LightChain) InsertHeaderChain(data types.HeaderInsertDataBatch, checkFreq int) (int, error) {
	start := time.Now()
	if i, err := lc.hc.ValidateHeaderChain(data, checkFreq); err != nil {
		return i, err
	}
	...
	whFunc := func(data types.HeaderInsertData) error {
		...
		header := data.Header
		status, err := lc.hc.WriteHeader(data.Header)
		...
		return lc.hc.WriteValidators(data.Validators)
	}
	i, err := lc.hc.InsertHeaderChain(data, whFunc, start)
...
func (lc *LightChain) GetHeaderAndValidatorsByNumber(number uint64) (types.HeaderInsertData, error) {
	header := lc.GetHeaderByNumber(number)
	if header == nil {
...
func (lc *LightChain) GetHeaderAndValidatorsByHash(hash common.Hash) (types.HeaderInsertData, error) {
...
func (lc *LightChain) GetHeaderAndValidators(hash common.Hash, number uint64) (types.HeaderInsertData, error) {
	header := lc.GetHeader(hash, number)
	if header == nil {
		return types.HeaderInsertData{}, core.ErrUnknownBlock
	}
	return headerToHeaderInsertData(lc, header)
...
func (lc *LightChain) getValidatorsByHeader(header *types.Header) ([]common.Address, error) {
	root := header.DposContext.EpochRoot
	return lc.hc.GetValidators(root)
}
